...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="flex justify-between items-center w-full px-5 py-3.5 z-40 bg-[#1e0f10]/80 backdrop-blur-md border-b border-[#5b4040]/30 sticky top-0">
      <div className="flex items-center gap-3">
        ...
        >
          <img 
            ...
            className="w-8 h-8 rounded-lg object-cover shadow-lg border border-white/10 group-hover:scale-105 transition-transform" 
          />
          <div>
            <h1 className="font-bold text-lg leading-tight bg-clip-text text-transparent bg-gradient-to-r from-[#FF4B5C] to-[#6C4AB6] tracking-tight">
              MINGLE@MANIPAL
            </h1>
            <span className="text-[10px] text-[#e3bebd]/70 font-semibold tracking-wider uppercase block">
              Academic Network
            </span>
          </div>
        </button>
      ...
      <div className="flex items-center gap-2">
        ...
        {onOpenPreviews && (
          <button
            ...
            className="px-2.5 py-1.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/15 text-xs text-[#f9dcdb] font-semibold transition-all flex items-center gap-1 cursor-pointer"
        ...
        >
          ...
          <span>{hasCampusPass ? 'PASS ACTIVE' : 'CAMPUS PASS'}</span>
        </button>
        ...
        <button
          onClick={onOpenAdmin}
          title="Admin Panel (Passcode 3000)"
          className={`px-2.5 py-1.5 rounded-full border transition-all cursor-pointer flex items-center gap-1 text-xs font-bold ${
            currentView === 'admin'
              ? 'bg-[#FF4B5C]/30 text-[#ffb3b3] border-[#FF4B5C]'
              : 'bg-[#FF4B5C]/15 text-[#ffb3b3] border-[#FF4B5C]/30 hover:bg-[#FF4B5C]/25'
          }`}
        >
          <span className="material-symbols-outlined text-sm text-[#FF4B5C]">admin_panel_settings</span>
          <span className="hidden sm:inline">ADMIN (3000)</span>
        </button>
